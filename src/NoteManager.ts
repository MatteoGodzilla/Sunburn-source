...
export class NoteManager {
    ...
    private holdingLeftButton = false
    private holdingRightButton = false
    ...
    mouseHandler(ev: MouseEvent, app: PIXI.Application, noteRender: NoteRender, baseBPM: number) {
        ...
        if (ev.type === "mousedown") {
            if (ev.button === 0) this.holdingLeftButton = true
            if (ev.button === 2) this.holdingRightButton = true
            ...
            if (this.mode === Modes.add) {
                ...
                if (this.noteClass === NoteClass.TAP) {
                    ...
                    } else if ((lane === 1 || lane === 2) && ev.button === 0) {
                        //BLUE TAP
                        let present = false
                        for (let n of this.notes) {
                    ...
                    if (lane === -2 || lane === -1) {
                        ...
                        if (!present) {
                            if (ev.button === 0) data.type = noteTypes.SCR_G_UP
                            else if (ev.button === 2) data.type = noteTypes.SCR_G_DOWN
                        ...
                        if (!present) {
                            if (ev.button === 0) data.type = noteTypes.SCR_B_UP
                            else if (ev.button === 2) data.type = noteTypes.SCR_B_DOWN
                    ...
                    if (lane === -3 && ev.button === 0) {
                        data.type = noteTypes.BPM_FAKE
                        this.notes.push(data)
                    } else if ((lane === -2 || lane === -1) && ev.button === 0) {
                ...
                if (this.noteClass === NoteClass.TAP) {
                    ...
                    } else if ((lane === -1 || lane === 1) && ev.button === 0) {
                        for (let n of this.notes) {
            ...
            if (ev.button === 0) this.holdingLeftButton = false
            if (ev.button === 2) this.holdingRightButton = false
        }
