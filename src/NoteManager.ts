            ...
            if(ev.type === "mouseup"){
                if(ev.x > app.renderer.width/2 - noteRender.uiScale * 2.5 && ev.x < app.renderer.width/2 - noteRender.uiScale/2){
                    if(ev.which === 1){
                        ...
                        let match
                        for(let note of this.notes){
                            if(note.type === noteTypes.TAP_G && note.time === this.selectedTime) {match = note
                            break}
                        }
                    ...
                    if(ev.which === 1){
                        ...
                        let match
                        for(let note of this.notes){
                            if(note.type === noteTypes.TAP_R && note.time === this.selectedTime) {match = note
                            break}
                        }
                        if(match) this.notes.splice(this.notes.indexOf(match),1)
                    }
            ...
            if(ev.type === "mouseup" && ev.which === 1){
                ...
                if(ev.x > app.renderer.width/2 - noteRender.uiScale * 2.5 && ev.x < app.renderer.width/2 - noteRender.uiScale/2){
                    searchList = [
                        noteTypes.TAP_G,
                        noteTypes.SCR_G_UP,
                        noteTypes.SCR_G_DOWN,
                        noteTypes.SCR_G_ANYDIR,
                        noteTypes.CF_SPIKE_G,
                    ]
                } else if(ev.x >= app.renderer.width/2 + noteRender.uiScale/2 && ev.x < app.renderer.width/2 + noteRender.uiScale * 2.5){
                    searchList = [
                        noteTypes.TAP_B,
                        noteTypes.SCR_B_UP,
                        noteTypes.SCR_B_DOWN,
                        noteTypes.SCR_B_ANYDIR,
                        noteTypes.CF_SPIKE_B
                    ]
                } else {
